<x-layouts::auth>
    <div class="flex flex-col gap-6">
        ...
        <div class="flex flex-col items-center gap-2 text-center">
            ...
            <h1 class="text-2xl font-bold">{{ __('Sign in') }}</h1>
            <p class="text-base-content/70">{{ __('Ship Faster and Focus on Growth.') }}</p>

            <div class="mt-2">
                <span class="text-base-content/70 text-sm">{{ __('Login with') }}</span> <a href="#"
                    class="link link-primary no-underline font-semibold text-sm">{{ __('Magic Link') }}</a>
            </div>

            <!-- Optional: Login as User/Admin Buttons (Placeholder from design) 
            <div class="flex w-full gap-2 mt-4">
                <button class="btn btn-outline btn-primary flex-1">{{ __('Login as User') }}</button>
                <button class="btn btn-outline flex-1">{{ __('Login as Admin') }}</button>
            </div>
            -->
        </div>
        ...
        <form method="POST" action="{{ route('login.store') }}" class="flex flex-col gap-4">
            ...
            <div class="flex items-center justify-between mt-2">
                <label class="label flex cursor-pointer justify-start items-center gap-2 p-0">
                    <input type="checkbox" name="remember" class="checkbox checkbox-sm checkbox-primary rounded" {{ old('remember') ? 'checked' : '' }} />
                    <span class="label-text">{{ __('Remember Me') }}</span>
                </label>

                @if (Route::has('password.request'))
                    <a class="link link-primary no-underline text-sm font-semibold" href="{{ route('password.request') }}"
                        wire:navigate>
                        {{ __('Forgot Password?') }}
                    </a>
                @endif
        ...
        <div class="text-center text-sm">
            <span class="text-base-content/70">{{ __('New on our platform?') }}</span>
            <a href="{{ route('register') }}" class="link link-primary no-underline font-semibold"

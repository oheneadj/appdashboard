<div class="py-5 space-y-5 border border-slate-100 shadow-sm rounded-2xl bg-white" wire:cloak
    x-data="{ showRecoveryCodes: false }">
    <div class="px-6 space-y-2">
        <div class="flex items-center gap-2">
            <span class="icon-[tabler--lock] size-4 text-slate-500"></span>
            <h3 class="text-base font-bold text-slate-800">{{ __('2FA Recovery Codes') }}</h3>
        </div>
    ...
    <div class="px-6">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button x-show="showRecoveryCodes" class="btn btn-primary btn-sm" @click="showRecoveryCodes = false"
        ...
        <div x-show="showRecoveryCodes" x-transition id="recovery-codes-section" class="relative overflow-hidden"
            ...
            <div class="mt-3 space-y-3">
                ...
                @if (filled($recoveryCodes))
                    <div class="grid gap-1 p-4 font-mono text-sm rounded-xl bg-slate-50 border border-slate-100" role="list"
                        aria-label="{{ __('Recovery codes') }}">
                        @foreach($recoveryCodes as $code)
                            <div role="listitem" class="select-text text-slate-700"
